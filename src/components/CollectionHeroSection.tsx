...
// Иконки социальных сетей (вынесены в отдельные компоненты)
const OpenSeaIcon = () => (
...
export default function CollectionHeroSection({ 
  ...
  useEffect(() => {
    ...
    // Если переданы данные через пропсы, используем их
    if (propCollectionData) {
      setCollectionData(propCollectionData);
      setIsLoading(false);
    } else {
      // Иначе используем данные по умолчанию или загружаем из API
      // Здесь можно добавить запрос к API для получения данных коллекции
  ...
  // Определение класса цвета для процентов
